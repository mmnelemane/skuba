...
func TestHasAddonUpdate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			aviu: AddonVersionInfoUpdate{
				Current: kubernetes.AddonsVersion{
					kubernetes.Cilium:  &kubernetes.AddonVersion{Version: "1.6.6", ManifestVersion: 3},
				...
				Updated: kubernetes.AddonsVersion{
					kubernetes.Cilium: &kubernetes.AddonVersion{Version: "1.6.6-rev5", ManifestVersion: 4},
				},
			},
			expected: true,
		},
...
func ExamplePrintAddonUpdates() {
	...
	// Output:
	//   - cilium: 1.6.6-rev5 (manifest version from 3 to 4)
	//   - dex: 2.16.0 -> 2.17.0
